...
- We can speedup the process with pipeline parallism
...
- Often, work can be split into different tasks
    - e.g., one task per input file
- These tasks can be processed independently
- Tasks may have different "sizes" (time for processing, required RAM, ...)
- Tasks may have dependencies (output of one task is input for another)
...
::: {.column width="50%"}
![](img/00-outline/manager-worker.svg){width="100%"}
:::
...
::: {.column width="50%"}
![](img/00-outline/wikipedia-matrix-multiplication.svg){width=100%}
...
- **Regularly structured data** (e.g., vectors, matrices, tensors) ...
    - ... have **natural decompositions** ...
    - ... and operations can be easily parallelized.
- Common applications:
    - Linear algebra / graphics processing
    - "Deep" learning etc.
